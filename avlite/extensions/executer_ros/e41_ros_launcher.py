...
class Collector(Node):
    ...
    def __init__(self, q: queue.Queue, collection_dt: float = 0.1):
        ...
        plan_subscription = self.create_subscription(String, 'local_plan', self.listener_callback, 10)
        control_subscription = self.create_subscription(String, 'control', self.listener_callback, 10)
...
class ROSExecuter(Executer):
    ...
    def step(self, control_dt=0.01, replan_dt=0.01, sim_dt=0.01, call_replan=True, call_control=True, call_perceive=True) -> None:
        ...
        if not self.ros_started:
            self.replan_dt = replan_dt; self.control_dt = control_dt; self.perception_dt = 0
            self._start_ros()
    ...
    def _start_ros(self):
        ...
        planner_path = os.path.join(current_dir, 'e43_planner_node.py')
        self.planner_process = subprocess.Popen(
            ['python3', planner_path, '--ros-args', '-p', f"planner_name:=my_planner", '-p', f"replan_dt:={self.replan_dt}"], 
        ...
        control_path = os.path.join(current_dir, 'e44_control_node.py')
        self.control_process = subprocess.Popen( ['python3', control_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE, preexec_fn=os.setsid)
        ...
        self.collector_node = Collector(self.q, collection_dt = 0.1)
