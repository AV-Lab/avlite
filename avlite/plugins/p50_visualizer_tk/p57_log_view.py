...
from avlite.plugins.p50_visualizer_tk.p53_ui_lib import attach_schema_tooltip, attach_tooltip, BUTTON_TOOLTIPS
...
class LogView(ttk.LabelFrame):
    _LAYER_TOGGLES = (
        ("perception", "avlite.c10_perception", "p57_show_perceive_logs"),
        ("planning", "avlite.c20_planning", "p57_show_plan_logs"),
        ("control", "avlite.c30_control", "p57_show_control_logs"),
        ("execution", "avlite.c40_execution", "p57_show_execute_logs"),
        ("visualization", "avlite.plugins.p50_visualizer_tk", "p57_show_vis_logs"),
        ("common", "avlite.c60_common", "p57_show_common_logs"),
    )
    ...
    def __init__(self, root: VisualizerApp):
        ...
        self.root = root
        self.max_log_lines = self.root.setting.p57_max_log_lines
        ...
        cb_core = ttk.Checkbutton(
            self.controls_frame, text="Core", variable=self.root.setting.p57_show_core_logs, command=self.update_log_filter,
        )
        cb_core.pack(side=tk.LEFT)
        attach_schema_tooltip(cb_core, VisualizationSettings, "p57_show_core_logs")
        cb_plugins = ttk.Checkbutton(
            self.controls_frame, text="Plugins", variable=self.root.setting.p57_show_plugins_logs, command=self.update_log_filter,
        )
        cb_plugins.pack(side=tk.LEFT)
        attach_schema_tooltip(cb_plugins, VisualizationSettings, "p57_show_plugins_logs")
        self.cb_perceive = ttk.Checkbutton( self.controls_frame, text="Perception", variable=self.root.setting.p57_show_perceive_logs, command=self.update_log_filter,)
        self.cb_perceive.pack(side=tk.LEFT)
        attach_schema_tooltip(self.cb_perceive, VisualizationSettings, "p57_show_perceive_logs")
        self.cb_plan = ttk.Checkbutton( self.controls_frame, text="Planning", variable=self.root.setting.p57_show_plan_logs, command=self.update_log_filter,)
        self.cb_plan.pack(side=tk.LEFT)
        attach_schema_tooltip(self.cb_plan, VisualizationSettings, "p57_show_plan_logs")
        self.cb_control = ttk.Checkbutton( self.controls_frame, text="Control", variable=self.root.setting.p57_show_control_logs, command=self.update_log_filter,)
        self.cb_control.pack(side=tk.LEFT)
        attach_schema_tooltip(self.cb_control, VisualizationSettings, "p57_show_control_logs")
        self.cb_execute = ttk.Checkbutton( self.controls_frame, text="Execution", variable=self.root.setting.p57_show_execute_logs, command=self.update_log_filter,)
        self.cb_execute.pack(side=tk.LEFT)
        attach_schema_tooltip(self.cb_execute, VisualizationSettings, "p57_show_execute_logs")
        self.cb_vis = ttk.Checkbutton( self.controls_frame, text="Visualization", variable=self.root.setting.p57_show_vis_logs, command=self.update_log_filter,)
        self.cb_vis.pack(side=tk.LEFT)
        attach_schema_tooltip(self.cb_vis, VisualizationSettings, "p57_show_vis_logs")
        self.cb_common = ttk.Checkbutton( self.controls_frame, text="Common", variable=self.root.setting.p57_show_common_logs, command=self.update_log_filter,)
        self.cb_common.pack(side=tk.LEFT)
        attach_schema_tooltip(self.cb_common, VisualizationSettings, "p57_show_common_logs")
        ...
        self.root.setting.p57_log_view_expanded.trace_add("write", lambda *_: self._sync_expand_button())
        ...
        self.rb_db_debug.pack(side=tk.RIGHT)
        self.log_area = ScrolledText(self, state="disabled", height=self.root.setting.p57_log_view_default_height.get(), wrap=tk.WORD)
        ...
        self.after(self.root.setting.p57_log_pull_time, self.poll_log_queue)
    ...
    def update_log_view_height(self, reverse: bool = False):
        ...
        if reverse:
            self.root.setting.p57_log_view_expanded.set(not self.root.setting.p57_log_view_expanded.get())

        if self.root.setting.p57_log_view_expanded.get():
            self.log_area.configure(height=self.root.setting.p57_log_view_expended_height.get())
            log.debug("Log view expanded.")
        else:
            self.log_area.configure(height=self.root.setting.p57_log_view_default_height.get())
    ...
    def _sync_filter_state(self) -> None:
        ...
        s = self.root.setting
        self._filter_state = {
            "p57_show_perceive_logs": bool(s.p57_show_perceive_logs.get()),
            "p57_show_plan_logs": bool(s.p57_show_plan_logs.get()),
            "p57_show_control_logs": bool(s.p57_show_control_logs.get()),
            "p57_show_execute_logs": bool(s.p57_show_execute_logs.get()),
            "p57_show_vis_logs": bool(s.p57_show_vis_logs.get()),
            "p57_show_common_logs": bool(s.p57_show_common_logs.get()),
            "p57_show_core_logs": bool(s.p57_show_core_logs.get()),
            "p57_show_plugins_logs": bool(s.p57_show_plugins_logs.get()),
            "p57_disable_log": bool(s.p57_disable_log.get()),
    ...
    def update_log_filter(self):
        ...
        if not self._filter_state["p57_show_perceive_logs"]:
            self.log_blacklist.add("avlite.c10_perception")
        if not self._filter_state["p57_show_plan_logs"]:
            self.log_blacklist.add("avlite.c20_planning")
        if not self._filter_state["p57_show_control_logs"]:
            self.log_blacklist.add("avlite.c30_control")
        if not self._filter_state["p57_show_execute_logs"]:
            self.log_blacklist.add("avlite.c40_execution")
        if not self._filter_state["p57_show_vis_logs"]:
            self.log_blacklist.add("avlite.plugins.p50_visualizer_tk")
            self.log_blacklist.add("avlite.plugins.p50_visualizer_tk")
        if not self._filter_state["p57_show_common_logs"]:
    ...
    def poll_log_queue(self, max_per_poll: int = 20):
        ...
        if messages and not self._filter_state.get("log_to_file") and not self._filter_state.get("p57_disable_log"):
    ...
    def should_show_log(record_name: str, filter_state: dict[str, bool]) -> bool:
        ...
        for _key, prefix, attr in LogView._LAYER_TOGGLES:
            if record_name.startswith(prefix + "."):
                if not filter_state.get("p57_show_core_logs", True):
                    return False
                return filter_state.get(attr, True)

        pkg = plugin_package_from_logger(record_name)
        if pkg is not None:
            if not filter_state.get("p57_show_plugins_logs", True):
                return False
    ...
    #     
    #     # Schedule next poll
    #     self.after(self.root.setting.p57_log_pull_time, self.poll_log_queue)  # Slightly longer interval
    #             
