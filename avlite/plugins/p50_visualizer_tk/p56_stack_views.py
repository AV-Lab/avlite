...
class PerceivePlanControlView(ttk.Frame):
    def __init__(self, root: VisualizerApp):
        ...
        self.root.setting.p55_show_perception_extras.trace_add("write", lambda *_: self.toggle_perception_extras())
        self.toggle_perception_extras()  # sync initial checkbox state

    def toggle_perception_extras(self):
        if self.root.setting.p55_show_perception_extras.get():
...
class PerceptionFrame(ttk.LabelFrame):
    def __init__(self, root: VisualizerApp, view: ttk.Frame):
        ...
        show_occ = ttk.Checkbutton(self, text="Show", variable=self.root.setting.p55_show_occupancy_flow)
        show_occ.grid(row=0, column=1, padx=2)
        attach_schema_tooltip(show_occ, VisualizationSettings, "p55_show_occupancy_flow")
        extras_cb = ttk.Checkbutton(
            self, text="Extras", variable=self.root.setting.p55_show_perception_extras,
            command=lambda: self.root.perceive_plan_control_view.toggle_perception_extras(),
        )
        extras_cb.grid(row=0, column=2, padx=2)
        attach_schema_tooltip(extras_cb, VisualizationSettings, "p55_show_perception_extras")
    ...
    def update_data(self):
        ...
        allowed_default_plugins = set(PerceptionStrategy.registry.keys()) & set(AppSettings.c50_default_plugins)
        community_prefixes = tuple(plugin_module_prefix(a) for a in AppSettings.c50_community_plugins.keys())
...
class PlanFrame(ttk.LabelFrame):
    def __init__(self, root: VisualizerApp, view:ttk.Frame):
        ...
        global_show = ttk.Checkbutton(global_frame, text="Global", command=self.root.update_views, variable=self.root.setting.p55_global_plan_view)
        global_show.pack(side=tk.LEFT)
        attach_schema_tooltip(global_show, VisualizationSettings, "p55_global_plan_view")
        ...
        local_show = ttk.Checkbutton(wp_frame, text="Local", command=self.root.update_views, variable=self.root.setting.p55_local_plan_view)
        local_show.pack(side=tk.LEFT)
        attach_schema_tooltip(local_show, VisualizationSettings, "p55_local_plan_view")
        local_g = ttk.Checkbutton(wp_frame, text="G", variable=self.root.setting.p55_show_local_global_view, command=self.root.update_ui)
        local_g.pack(side=tk.LEFT)
        local_f = ttk.Checkbutton(wp_frame, text="F", variable=self.root.setting.p55_show_local_frenet_view, command=self.root.update_ui)
        local_f.pack(side=tk.LEFT)
        attach_schema_tooltip(local_g, VisualizationSettings, "p55_show_local_global_view")
        attach_schema_tooltip(local_f, VisualizationSettings, "p55_show_local_frenet_view")
