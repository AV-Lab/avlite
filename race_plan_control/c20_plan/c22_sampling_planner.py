...
class RNDPlanner(Planner):
    ...
    def replan(self, back_to_ref_horizon=10):
        ...
        # )
        # self.lattice.create_edges()
        # self.selected_local_plan = np.random.choice(self.lattice.next_edges) if len(self.lattice.next_edges) > 0 else None
        # edge: Optional[Edge] = self.selected_local_plan
        ...
        # self.next_edges = self.lattice.next_edges
        #
        # return
        ...
        self.selected_local_plan = np.random.choice(self.next_edges) if len(self.next_edges) > 0 else None
        edge: Optional[Edge] = self.selected_local_plan
        while edge is not None and len(edge.next_edges) > 0:
            edge.selected_next_edge = np.random.choice(edge.next_edges)
            edge = edge.selected_next_edge
