...
class RNDPlanner(Planner):
    ...
    def replan(self, sample_size=1, back_to_ref_horizon=10, sample=True):
        ...
        d1_ = 0
        ep1 = Edge(s,d, s1_, d1_, self.global_trajectory, num_of_points= back_to_ref_horizon+2)
        self.lattice_graph[(s1_,d1_)] = ep1

        s2_ = s1_ + 10
        d2_ = 5
        ep2 = Edge(s1_,d1_, s2_, d2_, self.global_trajectory, num_of_points= back_to_ref_horizon+2)
        ep1.append_next_edges(ep2)

        s3_ = s2_ + 10
        d3_ = 0
        ep3 = Edge(s2_,d2_, s3_, d3_, self.global_trajectory, num_of_points= back_to_ref_horizon+2)
