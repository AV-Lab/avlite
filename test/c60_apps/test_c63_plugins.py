...
def test_load_community_plugin_setting_missing_install_is_non_fatal(monkeypatch, tmp_path):
    monkeypatch.setenv("AVLITE_PLUGINS_DIR", str(tmp_path / "empty_plugins"))
    (tmp_path / "empty_plugins").mkdir()
    assert load_community_plugin_setting("no-such-plugin", "no-such-plugin") is None
...
def test_load_stack_settings_imports_community_plugins(dashed_plugin, monkeypatch, tmp_path):
        ...
        yaml.dump(
            {
                ...
                "plugins": {
                    "avlite_executer_ROS2": {"replan_dt": 0.42, "control_dt": 0.07},
                },
            }
        ),
        encoding="utf-8",
    ...
    assert cls.replan_dt == pytest.approx(0.42)
    assert cls.control_dt == pytest.approx(0.07)
