...
def is_road_reversed(road: ET.Element, root: ET.Element) -> bool:
    link = road.find("link")
    if link is not None:
        pred = link.find('predecessor')
        succ = link.find('successor')
        if pred is not None and pred.get("elementType") == "road" and pred.get("contactPoint") == "start":
            return True
        if succ is not None and succ.get("elementType") == "road" and succ.get("contactPoint") == "end":
            return True
    ...
    if root is not None:
        ...
        for junc in root.findall("junction"):
            for conn in junc.findall("connection"):
                if conn.get("connectingRoad") == road_id and conn.get("contactPoint") == "start":
                    return True
    return False
